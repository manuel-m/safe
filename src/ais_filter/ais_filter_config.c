...
int ais_filter_config_load(struct ais_filter_config_s* cfg_,const char* f_)
{
    ...
    /* ais_tcp_error */
    {
        lua_getglobal(L,"ais_tcp_error");
        if (!lua_istable(L, -1)) {
            MM_ERR("ais_tcp_error is not a table");
            return -1;
        }
        lua_getfield(L, -1, "max_connections");
        if (!lua_isnumber(L, -1)) {
            MM_ERR("ais_tcp_error.max_connections should be a number");
            return -1;
        }
        cfg_->ais_tcp_error.max_connections = (int) lua_tonumber(L, -1);
        lua_pop(L, 1);
        lua_getfield(L, -1, "port");
        if (!lua_isnumber(L, -1)) {
            MM_ERR("ais_tcp_error.port should be a number");
            return -1;
        }
        cfg_->ais_tcp_error.port = (int) lua_tonumber(L, -1);
        lua_pop(L, 1);
        lua_getfield(L, -1, "name");
        if (!lua_isstring(L, -1)) {
            MM_ERR("ais_tcp_error.name should be a string");
            return -1;
        }
        const char* s = lua_tostring(L, -1);
        cfg_->ais_tcp_error.name = strdup(s);
        lua_pop(L, 1);
